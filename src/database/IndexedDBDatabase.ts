...
class IndexedDBDatabase implements Database {
  public connect_database(): Promise<any> {
  ...
  public insert_records(record: Record): Promise<boolean> {
    return this.connect_database().then((db: IDBDatabase): Promise<boolean> => {
  ...
  public async query(startDate: number, endDate: number): Promise<Record[]> {
    // TODO Promise.all????
    const db = await this.connect_database()
