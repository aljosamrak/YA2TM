...
export class AnalyticsModule {
  constructor(@Optional() @SkipSelf() parentModule: AnalyticsModule, public ngGoogleAnalytics: AnalyticsService) {
    if (parentModule) {
      throw new Error('AnalyticsModule is already loaded. Import it in the AppModule only')
    }
  }

  static forRoot(config: AnalyticsIdConfig): ModuleWithProviders<AnalyticsModule> {
    return {
